...
public class Fractal {
	...
	private void drawKochCurve(Point2D p1, Point2D p2, int level) {
		...
		// Koch subdivision rule: ___ ->  _/\_
			...
			Turtle p1p2 = new Turtle(p1, p2);
			Double d12 = Math.sqrt(Math.pow(p1.getX()-p2.getX(),2) + Math.pow(p1.getY()-p2.getY(),2));
			p1p2.move(d12/3);
			Point2D p3 = p1p2.getPosition();
			p1p2.turnLeft(60); p1p2.move(d12/3);
			Point2D p4 = p1p2.getPosition();
			p1p2.turnRight(120); p1p2.move(d12/3);
			Point2D p5 = p1p2.getPosition();
	...
	private void drawTree(Point2D p1, Point2D p2, int level) {
		if(level==0) {	// base case
			...
			p1p2 = new Turtle(p1, p2);
			d12 = Math.sqrt(Math.pow(p1.getX()-p2.getX(),2) + Math.pow(p1.getY()-p2.getY(),2));
			p1p2.move(d12/3);
			p3 = p1p2.getPosition();
			drawLine(p1,p3);
			p1p2.turnRight(15); 
	...
	private void drawSierpinskiTriangle(Point2D p1, Point2D p2, Point2D p3, int level) {
		if(level==0) {	// base case
			...
			p4.setLocation((p1.getX()+p2.getX())/2, (p1.getY()+p2.getY())/2);
			
			Point2D p5 = new Point2D.Double();
			p5.setLocation((p2.getX()+p3.getX())/2, (p2.getY()+p3.getY())/2);
			
			Point2D p6 = new Point2D.Double();
			p6.setLocation((p3.getX()+p1.getX())/2, (p3.getY()+p1.getY())/2);
			
			//drawTriangle(p1,p4,p6);
			//drawTriangle(p2,p4,p5);
			//drawTriangle(p3,p5,p6);
	...
	private void drawSierpinskiCarpet(Point2D p, double a, int level) {
		if(level==0) {	// base case
			...
			p1.setLocation(p.getX() + 0,p.getY() + a/3);
			
			Point2D p2 = new Point2D.Double();
			p2.setLocation(p.getX() + 0,p.getY() +2*a/3);
			
			Point2D p3 = new Point2D.Double();
			p3.setLocation(p.getX()+a/3,p.getY()+2*a/3);
			
			Point2D p4 = new Point2D.Double();
			p4.setLocation(p.getX()+2*a/3,p.getY()+2*a/3);
			
			Point2D p5 = new Point2D.Double();
			p5.setLocation(p.getX()+ 2*a/3,p.getY()+a/3);
			
			Point2D p6 = new Point2D.Double();
			p6.setLocation(p.getX()+2*a/3,p.getY()+ 0);
			
			Point2D p7 = new Point2D.Double();
			p7.setLocation(p.getX()+a/3,p.getY()+0);
